...
### Если ключ шифрования утёк

`FERNET_KEY` защищает сразу всё: токены ботов, ключи от касс клиентов,
секреты вебхуков и сессии веб-входа. Раньше пути отхода не было вовсе —
смена ключа разом разлогинивала всех и делала нечитаемыми чужие кассы.
Теперь это четыре шага:

1. новый ключ в `FERNET_KEY`, старый — в `FERNET_KEYS_RETIRED`;
2. перезапуск (процесс читает оба: шифрует новым, расшифровывает любым);
3. `python -m app.rotate_keys` — перешифровывает всё хранимое;
4. убрать старый ключ из `FERNET_KEYS_RETIRED`.

Шаг 3 не нужен для работы, но обязателен, чтобы старый ключ можно было
выбросить: пока им зашифрована хоть одна строка, он остаётся действующим
ключом от всех касс всех клиентов. Перед первой ротацией задай
`WEBHOOK_SECRET_KEY` отдельно — иначе смена основного ключа меняет секреты
всех вебхуков сразу.
