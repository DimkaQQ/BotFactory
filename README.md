...
| Казахстан | ioka, Processing.kz | обратным запросом в API |
...
Crypto Bot — см. `PLATFORM_PAYMENT_METHODS` в `.env.example`. Там же
`RENEWAL_PRICE_MINOR`: сколько стоит следующий период работы бота. Запуск
включает первый период, дальше приходит счёт; автосписаний с сохранённой
карты нет ни у кого — у Crypto Bot их не существует, а половина клиентов на
автосписании и половина на счетах это два разных продукта в поддержке.
Неоплаченный период не удаляет ничего: бот работает ещё `RENEWAL_GRACE_DAYS`
дней, потом уходит с эфира и возвращается от одной кнопки «Продлить».

### Бэкапы и мониторинг

`deploy/backup.sh` раз в сутки снимает базу, и **только если она
изменилась** собирает архив (база + загруженные файлы + `.env`), шифрует и
присылает в Telegram: копия на том же сервере копией не является.
`deploy/watchdog.sh` раз в пять минут проверяет API, контейнеры, диск и
свежесть бэкапа и пишет в Telegram только на смене состояния. Установка,
восстановление и то, чего сторож принципиально не умеет, — в
`deploy/ops.md`.

### Один воркер — не случайность

`api` запускается одним процессом uvicorn, и это требование, а не
недосмотр. В памяти процесса живут три вещи: реестр запущенных ботов,
очереди «одно сообщение за раз на чат» и периодический проход по
невыданным заказам. С `--workers N` очередь перестала бы держать порядок
сообщений в чате, а проход по невыданным пошёл бы в N копий и мог выдать
товар дважды при перезапуске.

Масштабировать сейчас нужно вертикально. Горизонтально — только после
того, как эти три вещи переедут во внешнее хранилище (Redis или
блокировки в самой БД).
