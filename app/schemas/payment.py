...
class PaymentSettingsOut(BaseModel):
    ...
    #: Корень адресов уведомлений. Нужен, чтобы показать адрес для кассы,
    #: которую человек настраивает ПРЯМО СЕЙЧАС, а не только для уже
    #: сохранённой: раньше подсказка «укажи этот адрес в кабинете» появлялась
    #: лишь после сохранения, то есть ровно после того момента, когда она
    #: была нужна.
    callback_base: str = ""
    ...
    ready: bool = False
    #: Готова И берёт настоящие деньги. Отдельно от `ready`, потому что
    #: тестовый режим включён по умолчанию: касса с заполненными ключами
    #: выглядит подключённой, а платежи по ней ненастоящие.
    live: bool = False
